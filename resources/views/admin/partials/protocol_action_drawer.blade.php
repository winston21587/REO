            ...
            <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-4 sm:pl-10"
                ...
                <div class="pointer-events-auto w-screen max-w-[320px] sm:max-w-sm flex flex-col h-full bg-white shadow-2xl">
                    ...
                    <div class="px-4 sm:px-6 py-4 sm:py-6 border-b border-slate-100 flex justify-between items-start bg-slate-50 flex-none text-left">
                        ...
                        <button type="button" @click="open = false" aria-label="Close protocol actions drawer" class="text-slate-400 hover:text-[#8B0000] hover:bg-slate-100 transition-colors w-8 h-8 flex-shrink-0 flex items-center justify-center rounded-full mt-0.5">
                    ...
                    <div class="flex-1 overflow-y-auto p-4 space-y-4">
                        ...
                        <div class="space-y-1">
                            <span class="block text-[10px] font-semibold uppercase tracking-wider text-slate-400 px-3 pb-1">
                                Review Progression
                            </span>

                            <!-- View Details -->
                            <a :href="p.view_url"
                                class="flex items-center gap-3 px-3.5 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-[#8B0000] rounded-xl transition-colors">
                            ...
                            <template x-if="p.has_deficiency">
                                <button type="button"
                                    ...
                                    class="w-full flex items-center gap-3 px-3.5 py-2.5 text-sm font-medium text-emerald-800 hover:bg-emerald-50/70 rounded-xl transition-colors text-left">
                            ...
                            <template x-if="!p.has_deficiency">
                                <button type="button"
                                    ...
                                    class="w-full flex items-center gap-3 px-3.5 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-[#8B0000] rounded-xl transition-colors text-left">
                        ...
                        <div class="space-y-1 pt-3 border-t border-slate-100">
                            ...
                            <template x-if="p.can_assign">
                                <div>
                                    ...
                                    <template x-if="!p.has_valid_review_type">
                                        <div title="Review Classification must be set before assigning reviewers." class="w-full flex items-center gap-3 px-3.5 py-2.5 text-sm font-medium text-slate-400 bg-slate-50/70 rounded-xl cursor-not-allowed select-none text-left">
                                            <i class="fas fa-users-slash w-4 text-center text-slate-300"></i> 
                                            <span>Assign Reviewer(s)</span>
                                            <span class="ml-auto text-[9px] font-bold bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full border border-slate-300 leading-none whitespace-nowrap">
                                    ...
                                    <template x-if="!p.is_or_verified">
                                        <div>
                                            <div title="Official Receipt must be received and verified before generating a Recommendation Letter."
                                                 class="flex items-center gap-3 px-3.5 py-2.5 rounded-xl select-none text-slate-300">
                                                ...
                                                <span class="ml-auto text-[9px] font-bold bg-amber-50 text-amber-900 px-2 py-0.5 rounded-full border border-amber-200/90 leading-none whitespace-nowrap">
                                                    OR Required
                                                </span>
                                            </div>
                                            <template x-if="p.already_notified">
                                                <button type="button" disabled
                                                    ...
                                                    class="w-full flex items-center gap-3 px-3.5 py-2.5 text-sm font-medium text-slate-400 bg-slate-50/70 rounded-xl cursor-not-allowed select-none text-left">
                                                    <i class="fas fa-clock w-4 text-slate-400 text-center"></i>
                                                    <span>Awaiting Receipt Submission</span>
                                                    <span class="ml-auto text-[9px] font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full border border-slate-200 leading-none whitespace-nowrap">
                                                        Notified
                                                    </span>
                                                </button>
                                            </template>
                                            <template x-if="!p.already_notified">
                                                <button type="button"
                                                    ...
                                                    class="w-full flex items-center gap-3 px-3.5 py-2.5 text-sm font-medium text-amber-900 bg-amber-50/50 hover:bg-amber-100/70 rounded-xl transition-colors text-left border border-amber-200/50">
                                                    <i class="fas fa-bell w-4 text-amber-600 text-center"></i>
                                                    <span>Send Receipt Reminder</span>
                                                    <span class="ml-auto text-[9px] font-semibold bg-amber-100/80 text-amber-900 px-2 py-0.5 rounded-full border border-amber-200 leading-none">
                                                        OR Required
                                                    </span>
                                                </button>
                                            </template>
                        ...
                        <div class="space-y-1 pt-3 border-t border-slate-100">
                            ...
                            <form :action="p.update_status_url" method="POST" :id="'revertPhaseForm-' + p.id">
                                ...
                                <button type="button"
                                    ...
                                    class="w-full flex items-center gap-3 px-3.5 py-2.5 text-sm font-medium text-slate-600 hover:text-rose-700 hover:bg-slate-100 rounded-xl transition-colors text-left border border-transparent">
