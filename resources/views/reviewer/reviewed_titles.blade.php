<x-reviewer_layout title="Reviewed Protocols">
    @php
        ...
        $exemptCount = $titles->where('Review_Type', 'Exempt Review')->count();
        $expeditedCount = $titles->where('Review_Type', 'Expedited Review')->count();
        $fullBoardCount = $titles->where('Review_Type', 'Full Board Review')->count();
    ...
    <div class="w-full space-y-6 pb-12"
         x-data="{
             ...
             viewMode: (window.innerWidth < 768 ? 'grid' : (localStorage.getItem('reo_reviewer_view_reviewed') || 'table')),
             init() {
             ...
             setView(mode) {
                 this.viewMode = mode;
                 localStorage.setItem('reo_reviewer_view_reviewed', mode);
             },
             resetFilters() {
                 this.search = '';
             ...
             hasActiveFilters() {
                 return this.search.trim().length > 0 || this.filterCategory !== 'all' || this.filterType !== 'all';
             },
             matches(item) {
            ...
            <div>
                <div class="flex items-center gap-2.5">
                    ...
                    <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">
                        ({{ $totalCount }} {{ Str::plural('Evaluated Protocol', $totalCount) }})
                    </span>
                </div>
                <p class="text-slate-500 mt-1 text-xs sm:text-sm max-w-2xl leading-relaxed">
                    Historical ledger of research protocols you have formally evaluated and submitted reviews for.
                </p>
            </div>

            <!-- Quick Metrics Strip -->
            <div class="flex items-center gap-2 sm:gap-3 flex-wrap">
                <div class="px-3.5 py-2 rounded-xl bg-white border border-slate-200/90 shadow-2xs flex items-center gap-2 text-xs">
                    <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
                    <span class="text-slate-500 font-medium">Total Evaluated:</span>
                    <span class="font-bold text-slate-900 tabular-nums">{{ $totalCount }}</span>
                </div>
                @if($fullBoardCount > 0)
                <div class="px-3.5 py-2 rounded-xl bg-white border border-slate-200/90 shadow-2xs flex items-center gap-2 text-xs">
                    <span class="w-2 h-2 rounded-full bg-indigo-500"></span>
                    <span class="text-slate-500 font-medium">Full Board:</span>
                    <span class="font-bold text-slate-900 tabular-nums">{{ $fullBoardCount }}</span>
                </div>
                @endif
                @if($expeditedCount > 0)
                <div class="px-3.5 py-2 rounded-xl bg-white border border-slate-200/90 shadow-2xs flex items-center gap-2 text-xs">
                    <span class="w-2 h-2 rounded-full bg-blue-500"></span>
                    <span class="text-slate-500 font-medium">Expedited:</span>
                    <span class="font-bold text-slate-900 tabular-nums">{{ $expeditedCount }}</span>
                </div>
                @endif
            </div>
        </div>
            ...
            <div class="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-2.5 sm:gap-3">
                ...
                <div class="relative flex-1 w-full min-w-0 lg:min-w-[260px]">
                    ...
                    <input type="text" 
                           ...
                           aria-label="Search evaluated protocols"
                ...
                <div class="flex items-center gap-2 sm:gap-2.5 w-full lg:w-auto">
                    ...
                    <!-- View Mode Switcher (Desktop Only: hidden on mobile) -->
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/80 border-b border-slate-200/80 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                            ...
                            <th class="py-3.5 px-4 hidden sm:table-cell whitespace-nowrap">Submission Date</th>
                        ...
                        <tr x-show="matches({
                            ...
                            <td class="py-4 px-4 align-middle max-w-md">
                                ...
                                <div class="flex items-center gap-2 mt-1.5 md:hidden text-[11px] text-slate-500 flex-wrap">
                                    ...
                                    <span>•</span>
                                    <span class="font-bold uppercase tracking-wider {{ $revTypeTextColor }}">{{ $title->Review_Type ?? 'Unassigned' }}</span>
                                </div>
                            </td>
        ...
        <div x-show="viewMode === 'grid' || isMobile" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
            ...
            <div x-show="matches({
                ...
                <div class="mb-3.5 flex items-center justify-between gap-2">
                    <span class="text-xs font-bold uppercase tracking-wider text-emerald-700 truncate">
                        Evaluated
                    </span>
                ...
                <div class="space-y-2 mb-5 text-xs">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs font-bold uppercase tracking-wider {{ $revTypeTextColor }} truncate">{{ $title->Review_Type ?? 'Unassigned' }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-slate-500 text-[11px]">
                        ...
                        <span class="truncate">Submitted: {{ $title->created_at ? $title->created_at->format('M d, Y') : '—' }}</span>
                    </div>
        ...
        <div class="py-16 px-6 text-center bg-white rounded-2xl border border-slate-200/90 shadow-2xs max-w-2xl mx-auto">
            <div class="w-14 h-14 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-center mx-auto mb-4 text-slate-400 shadow-2xs">
                <i class="fas fa-box-archive text-2xl" aria-hidden="true"></i>
            </div>
            <h3 class="font-bold text-base text-slate-900 tracking-tight font-heading">
                No Reviewed Protocols Found
            </h3>
            <p class="text-xs sm:text-sm text-slate-500 mt-1.5 max-w-md mx-auto leading-relaxed">
                You have not yet completed and submitted evaluations for any assigned research protocols. Completed reviews will be safely archived here for your reference.
            </p>
