...
(async () => {
    console.log('--- Starting /impeccable layout visual verification ---');
    ...
    const mainTableShot = path.join(targetDir, 'layout_active_protocols_desktop.png');
    await page.screenshot({ path: mainTableShot });
    console.log('Saved main table shot:', mainTableShot);

    // Test 2: Open Protocol Action Drawer on first row to verify semantic zones
    const actionBtn = await page.$('table tbody tr:first-child button[title="Protocol Actions"]');
    if (actionBtn) {
        console.log('Clicking action button to open drawer...');
        await actionBtn.click();
        await page.waitForTimeout(600);
        const drawerShot = path.join(targetDir, 'layout_drawer_grouped_zones.png');
        await page.screenshot({ path: drawerShot });
        console.log('Saved drawer grouped shot:', drawerShot);
    ...
    // Test 3: Active Filters Chips Bar
    ...
    const filterChipsShot = path.join(targetDir, 'layout_filter_chips_desktop.png');
    await page.screenshot({ path: filterChipsShot });
    console.log('Saved filter chips shot:', filterChipsShot);

    // Test 4: Mobile Adaptive Cards View (390x844)
    console.log('Testing mobile cards viewport...');
    ...
    const mobileShot = path.join(targetDir, 'layout_active_protocols_mobile.png');
    await mobilePage.screenshot({ path: mobileShot });
    console.log('Saved mobile shot:', mobileShot);
