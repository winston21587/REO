        ...
        @forelse($pendingSubmissions as $sub)
            <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all group">
                ...
                <div class="flex items-center justify-end gap-3 mt-4">
                    ...
                    <button
                        onclick="openTriageModal('{{ $sub->id }}', '{{ addslashes($sub->Study_Protocol_title) }}', '', '{{ $sub->or_file_path ? asset($sub->or_file_path) : '' }}', {{ $sub->is_or_verified ? 'true' : 'false' }}, '{{ $sub->cv_verification_status ?? '' }}', {{ $sub->project_type ? 'true' : 'false' }}, '{{ addslashes($sub->researcher->user->first_name ?? '') }} {{ addslashes($sub->researcher->user->last_name ?? '') }}', '{{ $sub->researcher->user->email ?? '' }}')"
